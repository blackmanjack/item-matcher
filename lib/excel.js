 ...
 * Kalau tiap row punya `_imageAbsPath` (path absolut file gambar di disk, sudah divalidasi
 * caller), gambar ditempel di kolom pertama ("Gambar"). Key `_imageAbsPath` sendiri tidak
 * ikut jadi kolom data.
 */
async function writeExcelBuffer(rows, sheetName = 'Hasil') {
  ...
  if (rows.length > 0) {
    const hasImages = rows.some((r) => r._imageAbsPath);
    const dataKeys = Object.keys(rows[0]).filter((k) => k !== '_imageAbsPath');
    ...
    rows.forEach((row) => {
      ...
      excelRow.height = 90;
      const imgPath = row._imageAbsPath;
      if (!imgPath) return;

      const excelExt = EXCEL_IMAGE_EXT[path.extname(imgPath).toLowerCase()];
      if (!excelExt) return; // format tidak didukung exceljs (mis. webp) - lewati saja

      try {
        const imageId = workbook.addImage({ buffer: fs.readFileSync(imgPath), extension: excelExt });
        worksheet.addImage(imageId, {
          tl: { col: 0, row: excelRow.number - 1 },
          ext: { width: 90, height: 90 },
        });
      } catch {
        // file gambar hilang/rusak - lewati, jangan gagalkan seluruh export
      }
    });
  }
